...
// Transcendence: on startup load spork values from previous session if they exist in the sporkDB
...
void ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "spork") {
        ...
        // Transcendence: add to spork database.
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_17_MAGIC_HEADER_UPDATE) r = SPORK_17_MAGIC_HEADER_UPDATE_DEFAULT;
